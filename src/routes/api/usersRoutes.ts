...
type UserSnapshot = { user_id: string; email: string; role: string; role_id: string | null; is_active: boolean; name: string | null; role_name: string | null };

async function fetchUserSnapshot(userId: string): Promise<UserSnapshot | null> {
  const rows = await query<UserSnapshot>(
    `SELECT u.user_id, u.email, u.role, u.role_id, u.is_active, u.name, r.name AS role_name
