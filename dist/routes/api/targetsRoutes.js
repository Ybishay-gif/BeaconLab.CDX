...
function parseActivityLeadType(query) {
...
targetsRoutes.post("/targets", requireRole(["admin", "planner"]), async (req, res, next) => {
    try {
        ...
        appendChangeLog({ userId: req.user.userId, email: req.user.email }, { objectType: "target", objectId: result.targetId, action: "create", after: { targetId: result.targetId, planId } }).catch(console.error);
...
targetsRoutes.put("/targets/:targetId", requireRole(["admin", "planner"]), async (req, res, next) => {
    try {
        ...
        appendChangeLog({ userId: req.user.userId, email: req.user.email }, { objectType: "target", objectId: req.params.targetId, action: "update", before, after, metadata: { planId: parsePlanId(req.query), activityLeadType: req.query.activityLeadType || undefined } }).catch(console.error);
        res.json({ ok: true });
    }
    catch (error) {
        next(error);
...
targetsRoutes.post("/targets/populate-defaults", requireRole(["admin", "planner"]), async (req, res, next) => {
    try {
        ...
        const count = await batchCreateTargets(defaults.map((d) => ({
            state: d.state,
            segment: d.segment,
            source: d.source,
            targetValue: d.target_value,
        })), req.user.userId, planId, alt);
        appendChangeLog({ userId: req.user.userId, email: req.user.email }, { objectType: "target", objectId: planId, action: "populate_defaults", metadata: { planId, defaultTargetKey, activityLeadType: alt, count } }).catch(console.error);
