...
function normalizeCorTargetInput(value) {
...
function parsePriceDecisionOverrides(raw, activityLeadType) {
...
function normalizeUpliftLimit(raw, fallback = 0.1) {
...
function chooseRecommendedTestingPoint(rows, rule) {
    ...
    const baselineTestingPoint = Number(rows.find((row) => Number(row.testing_point) === 0)?.testing_point ?? 0);
    if (!rule) {
        const fallbackByFlag = rows.find((row) => Number.isFinite(Number(row.recommended_testing_point)) &&
...
async function getPriceDecisionOverridesForPlan(planId, activityLeadType) {
    const normalizedPlanId = String(planId || "").trim();
    if (!normalizedPlanId) {
        return new Map();
    }
...
async function getStateSegmentPerformanceFromDaily(filters, normalized) {
...
async function listStateSegmentFiltersFromDaily(normalized) {
 ...
 */
async function getPriceExplorationBQ(normalized) {
...
export async function getPriceExploration(filters) {
    ...
    if (!strategyRules.length) {
        ...
        const groupedWithoutRules = new Map();
    ...
    for (const groupRows of grouped.values()) {
        ...
        const state = String(sample.state || "").toUpperCase();
        const segment = extractSegmentFromChannelGroup(String(sample.channel_group_name || ""));
        const matchedRule = strategyRules.find((rule) => rule.states.includes(state) && rule.segments.includes(segment)) || null;
        const overrideKey = buildPriceDecisionOverrideKey(String(sample.channel_group_name || ""), state, segment);
        const overrideTp = manualOverrides.get(overrideKey) ?? manualOverrides.get(buildPriceDecisionOverrideKey(String(sample.channel_group_name || ""), state, ""));
...
export async function getStrategyAnalysis(filters) {
    ...
    for (const groupRows of rowsByChannelStateSegment.values()) {
        ...
        const recommendedByFlag = groupRows.find((row) => Number.isFinite(Number(row.recommended_testing_point)) &&
            Number(row.testing_point) === Number(row.recommended_testing_point));
        const recommended = recommendedByFlag ||
            [...groupRows].sort((a, b) => {
                ...
                const aPriority = aTp !== 0 && Number.isFinite(aCpbUplift) && aCpbUplift <= 0.10 && aAdditionalClicks > 0
                    ? 0
                    : aTp === 0
                        ? 1
                        : 2;
                const bPriority = bTp !== 0 && Number.isFinite(bCpbUplift) && bCpbUplift <= 0.10 && bAdditionalClicks > 0
                    ? 0
                    : bTp === 0
                        ? 1
                        : 2;
        ...
        const derivedBaselineWr = Number.isFinite(expectedWr) && Number.isFinite(wrUplift) && Math.abs(1 + wrUplift) > 1e-9
            ? expectedWr / (1 + wrUplift)
            : null;
        const derivedBaselineCpc = Number.isFinite(expectedCpc) && Number.isFinite(cpcUplift) && Math.abs(1 + cpcUplift) > 1e-9
        ...
        recommendedSummaries.push({
            ...
            wr_weight: Number.isFinite(baselineBids) && baselineBids > 0
                ? baselineBids
            ...
            cpc_weight: Number.isFinite(baselineSold) && baselineSold > 0
                ? baselineSold
            ...
            cpb_weight: Number.isFinite(baselineBinds) && baselineBinds > 0
                ? baselineBinds
    ...
    const result = rules.map((rule) => {
        ...
        const wrRollup = matchingRecommended.reduce((acc, row) => {
        ...
        const cpbRollup = matchingRecommended.reduce((acc, row) => {
        ...
        const wr_uplift = wrRollup.weight > 0 && wrRollup.currentWins > 0
        ...
        const cpc_uplift = Number.isFinite(currentCpc) && Number.isFinite(expectedCpc) && Number(currentCpc) > 0
            ? (Number(expectedCpc) - Number(currentCpc)) / Number(currentCpc)
            : null;
        const cpb_uplift = cpbRollup.currentCost > 0
...
function aggregateStrategySlice(baselineRows, recommendedRows, qbc = 0) {
    ...
    const currentCpb = binds > 0 ? totalSpend / binds : null;
    const expectedCpb = binds + additionalBinds > 0 ? expectedTotalCost / (binds + additionalBinds) : null;
    const wrRollup = recommendedRows.reduce((acc, row) => {
        const current = Number(row.current_wr);
        const expected = Number(row.expected_wr);
        const weight = Number(row.wr_weight) || 0;
        if (weight > 0 && Number.isFinite(current) && Number.isFinite(expected)) {
    ...
    const cpbRollup = recommendedRows.reduce((acc, row) => {
        const current = Number(row.current_cpb);
        const expected = Number(row.expected_cpb);
        const weight = Number(row.cpb_weight) || 0;
        if (weight > 0 && Number.isFinite(current) && Number.isFinite(expected)) {
    ...
    const wrUplift = wrRollup.currentWins > 0 ? (wrRollup.expectedWins - wrRollup.currentWins) / wrRollup.currentWins : null;
    const expectedClicks = bids + additionalClicks;
    const expectedCpc = expectedClicks > 0 ? expectedTotalCost / expectedClicks : null;
    const cpcUplift = Number.isFinite(Number(cpc)) && Number.isFinite(Number(expectedCpc)) && Number(cpc) > 0
        ? (Number(expectedCpc) - Number(cpc)) / Number(cpc)
        : null;
    const cpbUplift = cpbRollup.currentCost > 0 ? (cpbRollup.expectedCost - cpbRollup.currentCost) / cpbRollup.currentCost : null;
    return {
        rule_name: "Aggregate",
        states: [],
        segments: [],
...
export async function getStateAnalysis(filters) {
    ...
    const stateRows = ALL_US_STATE_CODES.map((stateCode) => {
    ...
    const stateDetails = stateRows.map((stateRow) => {
        const segmentSet = stateRow.rule_name
            ? new Set((ruleRows.find((rule) => rule.rule_name === stateRow.rule_name)?.segments || []).map((value) => String(value || "").toUpperCase()))
            : null;
